...
<?php 
  if(isset($loggedIn)){
      if ($loggedIn) {
          ...
          $personalizedresult = mysqli_query($conn, $sql);
          if (!$personalizedresult) {
              echo "Database error: " . mysqli_error($conn); 
              die();
          }
          $personalized = mysqli_fetch_assoc($personalizedresult);
          echo '<h2 id="welcomeBack">Welcome Back ' . $personalized['firstName'] . ' ' . $personalized['lastName'] . '!</h2>';
      }
  }
  ?>
  <div class="container">
