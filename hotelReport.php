    ...
    <div class="container-hotel">
        <form action="connect.php" method="post">
            ...
            <div class="input-boxes">
                ...
                echo '<h3><i  class="fa-solid fa-people-group fa-3x"></i> 
                <div>
                ...
                $result = mysqli_query($conn, $query);
                $count = mysqli_fetch_array($result);
                echo '<h3 class="flex"><i class="fa-solid fa-calendar-days fa-3x"></i> 
                <div class="a">
                Total number of bookings for the hotel : 
                </div> <div class="value-right">
                 ' . $count[0] . ' </div> </h3>';
                ...
                if ($standard[0] > $deluxe[0] && $standard[0] > $suite[0]) {
                    echo '<h3>The most popular room is<br> the standard room.</h3>';
                }
                elseif ($deluxe[0] > $standard[0] && $deluxe[0] > $suite[0]) {
                    echo '<h3>The most popular room is<br> the deluxe room.</h3>';
                }
                elseif ($suite[0] > $standard[0] && $suite[0] > $deluxe[0]) {
                    echo '<h3>The most popular room is<br> the suite.</h3>';
                }
                elseif ($standard[0] == $deluxe[0] && $standard[0] > $suite[0]) {
                    echo '<h3 class="room">The most popular rooms are <br>the standard and deluxe rooms. </h3>';
                }
                elseif ($standard[0] == $suite[0] && $standard[0] > $deluxe[0]) {
                    echo '<h3>The most popular rooms are<br> the standard rooms and the suites.';
                }
                elseif ($suite[0] == $deluxe[0] && $suite[0] > $standard[0]) {
                    echo '<h3>The most popular rooms are<br> the deluxe rooms and the suites.';
                }

                $query = "SELECT AVG(cost) AS AveragePrice FROM Bookings WHERE hotelName='$hotelName'";
                $result = mysqli_query($conn, $query);
                $row = mysqli_fetch_assoc($result);
                echo '<h3 class="flex-2"><i class="fa-solid fa-hand-holding-dollar fa-3x"></i>
                <div class="a">
                The average price a tourist spends on a <div class="b">
                booking at your hotel : 
                </div> 
                </div>
                  <div class="value-right-2">
                  ' . round($row['AveragePrice'], 2) .'</div>  ';
