...
//   $query = "SELECT Hotels.*, COUNT(Visits.hotelName) AS views FROM Hotels LEFT JOIN Visits ON Hotels.hotelName = Visits.hotelName WHERE profileVerified=1 GROUP BY Hotels.hotelName ORDER BY views DESC";
//   $result = mysqli_query($conn, $query);
//   if (!$result) {
//       echo "Database error: " . mysqli_error($conn); 
//       die();
//   }
//   $personalized = mysqli_fetch_assoc($result);

//   SELECT Hotels.*, COUNT(Visits.hotelName) AS views FROM Hotels LEFT JOIN Visits ON Hotels.hotelName = Visits.hotelName WHERE profileVerified=1 GROUP BY Hotels.hotelName ORDER BY views DESC
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <form action="search.php" method="POST" id="searchSubmit()">
        ...
        <div class="list-container">
            <div class="left-col">            
                ...
                // new addition >
                elseif(isset($loggedIn) && $loggedIn==true){
                    echo '<h1>Based on your interests</h1>';
                        $query = mysqli_query($conn, "SELECT Hotels.*, COUNT(Visits.hotelName) AS views FROM Hotels LEFT JOIN Visits ON Hotels.hotelName = Visits.hotelName WHERE profileVerified=1 GROUP BY Hotels.hotelName ORDER BY views DESC, COUNT(Visits.user='" . $_SESSION['email'] . "')");
                        $count = mysqli_num_rows($query);
                        if($count == 0){
                            $output = "There were no matching results.";
                        }
